...
#define FB_CONVERT_ENTRY_TO_WSTRING(_srctype_) FB_CONVERT_ENTRY_FROM_STRING_TYPE(std::wstring, _srctype_)
		...
		template <typename T>
		variant::variant(const T& x, bool) {
			assign(x, true);
		}

		template <typename T>
		variant::variant(const T& x) {
			assign(x);
		}



		variant& variant::assign(const variant& x) {
			object = x.object;
			lessthan = x.lessthan;
			return *this;
		}
		...
		template<class T>
		variant& variant::assign(const T& x) {
		...
		// assignment operator 
		template<typename T>
		variant& variant::operator=(T const& x) {
			return assign(x);
		}

		// utility functions
		variant& variant::swap(variant& x) {
		...
		// comparison function
		bool variant::operator<(const variant& rh) const {
		...
		const std::type_info& variant::get_type() const {
			return object.type();
		}

		template<typename T>
		bool variant::can_be_type() const {
		...
		template<typename T>
		bool variant::is_of_type() const {
			return (get_type() == typeid(T));
		}


		template<typename T>
		T variant::cast() const {
			if (get_type() != typeid(T)) {
		...
		template<typename T>
		const T variant::convert_cast_impl() const {
			return cast<T>();
		}
